...
%	a bipolar electrode pair, modelled by the H-H model.
%   
%   MultiVar4_5b(Distance, I) function simulates the Hodgkin-Huxley model 
...
%   (in microA). As output it plots voltage(membrane potential)time series  
%   for the specified electrode distance from the fibre and current. 
%   
%   Example:
%   MultiVar4_5b([0.025],1000) for bipolar stimulation with anode then
%   cathode placed along fibre 
%   MultiVar4_5b([0.025],2000)for bipolar stimulation with cathode then
%   anode placed along fibre for cathode propagation (swop signs of defI in Ispan and elecPos 1 and 2)
%
...
%%  Running through the various input diameters 
    for i=1:length(Distance)
        ...
        [data,m,h,n,Ispan] = HHsim(Distance1,I,t,x,loop,xloop,dx);
        dataholder{1,i} = data;
    end
...
function [V,t,m,h,n,Ispan] = HHsim(z,defI,t,x,loop,xloop,dx)    
...
%%	Solver: Euler method
    for s=1:1:xloop-1
        for i=1:1:loop-1
            elecPos1 = ceil(loop/10000)+0.01;
            xe1(s,i) = dx*(s-elecPos1);
            elecPos2 = ceil(loop/10000)-0.01;
            xe2(s,i) = dx*(s-elecPos2);
        end
    end
            
    for i=1:1:loop-1
 %   Solving for membrane potential and gating variables for fibre length after stimulation site       
        for s=15:1:xloop-1
            v(s,i) = V(s,i) - Vrest;
...
%   Solving for membrane potential and gating variables for fibre length before stimulation site    
        for s=14:-1:1
            v(s,i) = V(s,i) - Vrest;
            alphaN(s,i) = (0.1-0.01*v(s,i))/(exp(1-0.1*v(s,i))-1);
            betaN(s,i) = 0.125*exp(-v(s,i)/80);
