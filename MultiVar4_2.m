...
%%  Running through the various input diameters 
	[data,t,synapsePos] = HHsim(Diameter,I,sType,t,x,loop,dt,xloop,dx);
...
function [v,t,synapsePos] = HHsim(defDiameter,defI,defsType,t,x,loop,dt,xloop,dx)    
...
%%	Solver: Euler method
    for i=1:loop-1
        for s=2:xloop-1
            ...
            t_0 = 16000;                 % Position (temporal) of synapse
            if s == synapsePos && i >= t_0      
                t0 = t_0*dt;            % presynaptic action potential arrival time (from running MultiVar4_1 for fibre of half the length of total xspan)
