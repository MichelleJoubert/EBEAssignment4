...
A       = 0.2; %Amplitude of input signal

f1      = 98; % frequency of input signal

PW      = 0.3; %Pulse width of square wave (Remember there is a limit to how wide this can be made depending on the frequency)
...
Is(M,:) = A*square(2*pi*f1*10^(-3)*t, DC)+A;  %Current Stimulation starting in the middle of the axon 
...
for i = 1:1:N
    ...
    Im = (d/(4*ri*dX*L))*(vm(j-1,i) - 2*vm(j,i) + vm(j+1,i))+ (Is(j,i)/(pi*d*L));
...
figure(1);
subplot(2,1,1);
plot(t,vm(M,:),t,200*Is(M,:));
title('Periodic pulsatile at F = 98 Hz');
subplot(2,1,2);
for p = 2:1:R
...
f1      = 96; % frequency of input signal
...
subplot(2,1,1);
plot(t,vm(M,:),t,200*Is(M,:));
title('Periodic pulsatile at F = 96 Hz');
...
f1      = 94; % frequency of input signal
...
subplot(2,1,1);
plot(t,vm(M,:),t,200*Is(M,:));
title('Periodic pulsatile at F = 94 Hz');
