% Title:     Hodgkin-Huxley propagation model entrainment with square pulses 
% Author:    Bernard Bauermeister (u10367561)
%            Script 1
% Goal: The goal of this script is to Simulate the propagation of action 
% potentials when a periodic pulsatile is used as an input. The max
% frequency where the model still entrains is determined.
...
A       = 2000; %Amplitude of input signal

f1      = 459; % frequency of input signal (Hz)

PW      = 0.5; %Pulse width of square wave (Remember there is a limit to how wide this can be made depending on the frequency)
 
DC      = PW*f1*0.1; %Duty cycle of square wave
...
Is(M,:) = 0.5*A*square(2*pi*f1*10^(-3)*t, DC) + 0.5*A;  %Current Stimulation starting in the middle of the axon 
...
figure(1);
subplot(2,1,1);
plot(t,vm(M,:),t,0.075*Is(M,:));
title('Periodic pulsatile at F = 459 Hz');
...
ylim([0,2100]);
title('AP propagation with a periodic pulsatile at F = 459 Hz');