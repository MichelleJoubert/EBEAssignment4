...
for i = 1:1:N
    ...
    f_e(j,i)     = ((d*dX)/(4*ri*L))*(((re*1.15*Ie(j,i))/(4*pi))*(((x_e1(j,i)^2)+(z1^2))^(-5/2))*(2*(x_e1(j,i)^2)-(z1^2)) - ((re*Ie(j,i))/(4*pi))*(((x_e2(j,i)^2)+(z2^2))^(-5/2))*(2*(x_e2(j,i)^2)-(z2^2)));
...
ylim([-30,4050]);
title('AP propagation with z = 0.05 cm, A^- = -10 mA/cm^2 and A^+ = 11.5 mA/cm^2');
xlabel('Distance along the axon [cm].');

subplot(1,2,2);
plot(x,f_e(:,201));
title('Activation function with z = 0.05 cm, A^- = -10 mA/cm^2 and A^+ = 11.5 mA/cm^2');
xlabel('Distance along the axon [cm].');
