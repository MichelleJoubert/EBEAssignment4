function [Done] = MultiVar4_1c(Diameter, I, Idur)
%   This function simulates the propagation of an action potential along   
%   nerve fibre of two different diameters, modelled by the Hodgkin-Huxley  
%	model.
...
%%  possible for high frequency blockade
%   Example:
%   MultiVar4_1c([0.01],1,1)
%   MultiVar4_1c([0.01,0.02,0.005],1,1)
%   MultiVar4_1c([0.01],10,40)
%
...
%%  Running through the various input diameters 
    for i=1:length(Diameter)
        ...
        [data,m,h,n,Ispan] = HHsim(Diameter1,I,Idur,t,x,loop,xloop,dx);
        dataholder{1,i} = data;
    end
...
function [V,t,m,h,n,Ispan] = HHsim(defDiameter,defI,defIdur,t,x,loop,xloop,dx)    
%     defI = 1;           % applied current or external stimulation (in microA)
%     defIdur= 5;         % duration of stimulation application (in msec)
...
%   current applied over the simulation period as specified by PulsePos vector    
    for i=1:length(PulsePos)
        ...
        Ispan(1,IspanPulsePos:PlusSpan) = Ispan(1,IspanPulsePos:PlusSpan)+defI;
        Ispan(12,IspanPulsePos:PlusSpan) = Ispan(12,IspanPulsePos:PlusSpan)+defI;
    end
